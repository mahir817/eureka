...
public class BuzzerController {
    ...
    public Buzzer createBuzzer(@PathVariable String username, @RequestParam String stream,
            @RequestParam String categoryLike, @RequestParam String difficulty, @RequestParam int count)
            throws Exception {
        ...
        List<Question> questions = questionService.findByStreamAndDifficultyLikeAndCategoryLike(stream, difficulty,
                categoryLike, pageable);
    ...
    public ResponseEntity<Buzzer> joinBuzzer(@PathVariable String username, @PathVariable String secretCode)
            throws Exception {
        ...
        if (buzzer.getPlayer2() != null)
            return new ResponseEntity<>(buzzer, HttpStatus.BAD_REQUEST);

        if (buzzer.getPlayer1() == player2)
            return new ResponseEntity<>(buzzer, HttpStatus.BAD_REQUEST);
    ...
    public Buzzer questionPassed(@PathVariable long id, @PathVariable int questionIndex) {
        ...
        if (questionIndex == qInDB)
            buzzer.setCurrentQuestion(questionIndex + 1);
        buzzerRepository.save(buzzer);
        return buzzer;
    }

    @PostMapping("/buzzer/{id}/{username}")
    public synchronized Buzzer buzzerBuzzer(@RequestParam int questionIndex, @RequestParam boolean correct,
            @RequestParam int score, @PathVariable String username, @PathVariable long id) throws Exception {
        ...
        if (!buzzer.getGameState().equals("IN_PROGRESS"))
            return buzzer;
        ...
        if (correct)
            buzzer.updatePlayerScore(player, score);
        else
            buzzer.updatePlayerScore(opponent, score);
    ...
    public synchronized Buzzer resultBuzzer(@PathVariable long id) throws Exception {
        ...
        if (buzzer.getGameState().equals("INACTIVE"))
            return buzzer;
    ...
    public Map<String, Object> getUserStats(@PathVariable String username) {
        ...
        List<Buzzer> games = buzzerRepository.findByPlayer1OrPlayer2(user, user);
        ...
        for (Buzzer game : games) {
            if (!"INACTIVE".equals(game.getGameState()))
                continue; // Only count finished games

            totalGames++;

            boolean isP1 = game.getPlayer1().equals(user);
            int myScore = isP1 ? game.getPlayer1Score() : game.getPlayer2Score();
            int oppScore = isP1 ? game.getPlayer2Score() : game.getPlayer1Score();
        ...
        stats.put("winRate", Math.round(winRate * 100.0) / 100.0); // Round to 2 decimal places
